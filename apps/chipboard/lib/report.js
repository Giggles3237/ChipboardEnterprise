...
function renderSheet(report, settings, { reportDate = new Date(), includeDisclosures = false } = {}) {
  ...
  const noProgram = attention.filter(u => u.status === 'no_program' && u.salePrice !== null);
  ...
  if (noProgram.length) {
    const noProgramRows = noProgram.map(u => `<tr data-brand="${brandOf(u.stockNumber, u.model)}">
        <td ${td('font-weight:bold;')}>${esc(u.stockNumber)}</td>
        <td ${td()}>${esc(u.model)}</td>
        <td ${td()}>${esc(u.color)}</td>
        <td data-v="${u.odometer}" ${td('text-align:right;')}>${milesFmt(u.odometer)}</td>
        <td data-v="${u.msrp || 0}" ${td('text-align:right;')}>${moneyFmt(u.msrp)}</td>
        <td data-v="${u.invoice || 0}" ${td('text-align:right;')}>${moneyFmt(u.invoice)}</td>
        <td data-v="${u.avp || 0}" ${td('text-align:right;')}>${moneyFmt(u.avp)}</td>
        <td data-v="${u.mileageAdj || 0}" ${td('text-align:right;')}>${moneyFmt(u.mileageAdj)}</td>
        <td data-v="${u.salePrice || 0}" ${td(`text-align:right;font-weight:bold;color:${BMW_BLUE};`)}>${moneyFmt(u.salePrice)}</td>
        <td data-v="${u.profit || 0}" ${td('text-align:right;')}>${moneyFmt(u.profit)}</td>
      </tr>`).join('');
    sections.push(`
    <div data-loaner-supplemental="true">
    <h2 style="font-family:${FONT};font-size:15px;color:${DARK};margin:28px 0 8px;">
      Calculated sale pricing - no lease program (${noProgram.length})
    </h2>
    <p style="font-family:${FONT};font-size:12px;color:${MUTED};margin:0 0 10px;">
      These units do not have a matching rate-sheet program, so no lease payment is shown. Sale price is still calculated from invoice, AVP, mileage adjustment, markup, and list-price cap.
    </p>
    <table role="presentation" cellpadding="0" cellspacing="0" class="sortable" style="border-collapse:collapse;font-family:${FONT};font-size:13px;color:${DARK};">
      <thead><tr>
        <th ${TH}>Stock #<span class="arw"></span></th><th ${TH}>Model<span class="arw"></span></th><th ${TH}>Color<span class="arw"></span></th>
        <th ${THR}>Miles<span class="arw"></span></th><th ${THR}>MSRP<span class="arw"></span></th><th ${THR}>Invoice<span class="arw"></span></th>
        <th ${THR}>AVP<span class="arw"></span></th><th ${THR}>Mileage Adj<span class="arw"></span></th><th ${THR}>Sale Price<span class="arw"></span></th><th ${THR}>Profit<span class="arw"></span></th>
      </tr></thead>
      <tbody>${noProgramRows}</tbody>
    </table>
    </div>`);
  }
