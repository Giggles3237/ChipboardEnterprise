...
class CompositeSalesEventPublisher {
...
class MysqlEventOutboxPublisher {
  constructor(pool) {
    this.pool = pool;
    this.ready = null;
  }
...
class SalesWebhookPublisher {
  async publish(event) {
    ...
    if (event.type === 'sale.created') {
      await sendSaleAddedWebhook(event.payload.sale, userData);
    }

    if (event.type === 'sale.deleted') {
      await sendSaleDeletedWebhook(event.payload.sale, userData);
    }
  }
}

class MysqlSalesAuditLogger {
  constructor(pool) {
    this.pool = pool;
    this.ready = null;
  }
...
module.exports = {
  CompositeSalesEventPublisher,
  MysqlEventOutboxPublisher,
  MysqlSalesAuditLogger,
  SalesWebhookPublisher
};
